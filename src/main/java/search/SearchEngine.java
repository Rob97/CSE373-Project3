...
public class SearchEngine {
	...
	public SearchEngine(String dataFolderName) {
		...
		this.pageRankAnalyzer = new PageRankAnalyzer(webpages, PAGE_RANK_DECAY, PAGE_RANK_EPSILON,
	...
	public double computeScore(IList<String> query, URI uri) {
		...
		if (pageRank <= 0.0) {
			throw new IllegalStateException(
					String.format("Page '%s' had a page rank of '%f'; all page ranks should be positive and non-zero.",
							uri, pageRank));
		}
	...
	private ISet<Webpage> collectWebpages(Path root) {
		try {
			return Files.walk(root).filter(Files::isRegularFile)
					...
					.map(Path::toUri).map(Webpage::load).collect(Bridge.toISet());
