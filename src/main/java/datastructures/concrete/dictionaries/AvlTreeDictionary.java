...
public class AvlTreeDictionary<K extends Comparable<K>, V> implements IDictionary<K, V> {
	...
	public AvlTreeDictionary(K key, V value, AvlTreeDictionary<K, V> left, AvlTreeDictionary<K, V> right,
			AvlTreeDictionary<K, V> parent) {
	...
	private AvlTreeDictionary<K, V> insert(AvlTreeDictionary<K, V> currentNode, K key, V value,
			AvlTreeDictionary<K, V> parentNode) {
		if (currentNode == null) {
	...
	private AvlTreeDictionary<K, V> putHelper(AvlTreeDictionary<K, V> current, K key, V value) {
		if (current.left == null && current.getKey().compareTo(key) > 0
				...
				|| current.left.getKey().compareTo(key) == 0 || current.right.getKey().compareTo(key) == 0) {
			return current;
...
/*
 * AvlTreeDictionary<K,V> currentNode = findHelper(root, key); V result =
 * currentNode.get(key); //has no children if(currentNode.left == null&&
 * currentNode.right == null) { currentNode = currentNode.parent;
 * if(currentNode.left.data.getKey().compareTo(key) == 0) { currentNode.left =
 * null; }else { currentNode.right = null; } //only has right child }else
 * if(currentNode.left == null) { currentNode.right.parent = currentNode.parent;
 * currentNode = currentNode.parent;
 * if(currentNode.left.data.getKey().compareTo(key) == 0) { currentNode.left =
 * currentNode.left.right; }else { currentNode.right = currentNode.right.right;
 * } }else if(currentNode.right == null) { currentNode.left.parent =
 * currentNode.parent; currentNode = currentNode.parent;
 * if(currentNode.left.data.getKey().compareTo(key) == 0) { currentNode.left =
 * currentNode.left.left; }else { currentNode.right= currentNode.right.left; }
 * }else { //nightmare: two full children AvlTreeDictionary<K,V> successor =
 * currentNode.right; while(successor.left != null) { successor =
 * successor.left; } currentNode.data = successor.data; currentNode =
 * successor.parent; currentNode.left = successor.right; } return result; }
 * 
 * }
 */